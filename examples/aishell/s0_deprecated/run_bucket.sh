...
node_rank=0
train_tool=train_bucket
infer_tool=recognize_bucket
# data
data=/apdcephfs/share_955219/speech/data/chinese
...
train_config=conf/train_conformer_bucket.yaml
cmvn=true
dir=exp/conformer_bucket
checkpoint=
...
if [ ${stage} -le 4 ] && [ ${stop_stage} -ge 4 ]; then
    ...
    for ((i = 0; i < $num_gpus; ++i)); do
    {
        ...
        python3 wenet/bin/${train_tool}.py --gpu $gpu_id \
            --config $train_config \
...
if [ ${stage} -le 5 ] && [ ${stop_stage} -ge 5 ]; then
    ...
    for mode in ${decode_modes}; do
    {
        ...
        mkdir -p $test_dir
        python3 wenet/bin/${infer_tool}.py --gpu 0 \
            --mode $mode \
...
if [ ${stage} -le 8 ] && [ ${stop_stage} -ge 8 ]; then
    ...
    # alignment output
    ali_result=$dir/ali
    python3 wenet/bin/alignment_deprecated.py --gpu -1 \
        --config $dir/train.yaml \
...
if [ ${stage} -le 9 ] && [ ${stop_stage} -ge 9 ]; then
    # just for observe the performance during training
    decode_checkpoint=$dir/142.pt
    # Specify decoding_chunk_size if it's a unified dynamic chunk trained model
    # -1 for full chunk
    decoding_chunk_size=
    ctc_weight=0.5
    ...
    test_dir=$dir/test_ctc_greedy_search
    mkdir -p $test_dir
    python3 wenet/bin/${infer_tool}.py --gpu 0 \
